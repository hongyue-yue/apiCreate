...
var consola$1 = _interopDefault(require('consola'));
...
var mkdir = function mkdir(dirpath, callback) {
...
var emptyDirs = function emptyDirs(dirpath) {
  var chunkList = fs.readdirSync(dirpath);
  chunkList.forEach(function (file) {
    fs.unlinkSync(dirpath + "/" + file);
  });
};
var writeFileSync = function writeFileSync(dirpath, data) {
  fs.writeFileSync(dirpath, data);
  consola.success("\u6587\u4EF6\u5199\u5165\u6210\u529F: ".concat(dirpath));
};
...
(function async() {
  ...
  var configFile = path$1.join(process.cwd(), "apiCreate.config.ts");
  cli.version(pkg.version).arguments("[cmd]").action(_async$1(function (cmd) {
    return _switch(cmd, [[function () {
      return "init";
    }, function () {
      return _await$1(fs$1.pathExists(configFile), function (_fs$pathExists) {
        return _invoke(function () {
          if (_fs$pathExists) {
            consola$1.info("\u68C0\u6D4B\u5230\u914D\u7F6E\u6587\u4EF6: ".concat(configFile));
          ...
          return  _await$1(fs$1.outputFile(configFile, configTemplate), function () {
            consola$1.success("写入配置文件完毕");
          });
        });
      });
    }], [function () {
      ...
      return _await$1(fs$1.pathExists(configFile), function (_fs$pathExists2) {
        if (!_fs$pathExists2) {
          return consola$1.error("\u627E\u4E0D\u5230\u914D\u7F6E\u6587\u4EF6: ".concat(configFile));
        }

        consola$1.success("\u627E\u5230\u914D\u7F6E\u6587\u4EF6: ".concat(configFile));
        return _catch(function () {
          ...
          return _await$1(generator.generate(), function (output) {
            spinner.stop();
            consola$1.success("yapi数据样本已获取，开始写入");
            generator.write(output, function (isNew) {
              if (isNew) {
                consola$1.success("api创建已完成");
              }
            });
          });
        }, function (err) {
          return consola$1.error(err);
        });
      });
    }]]);
  })).parse(process.argv);
